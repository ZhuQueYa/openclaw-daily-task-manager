...
| **XMind 里标了红旗，今日清单里却没有** | 生成今日计划时，**自动读取脑图红旗**并同步进任务池，长期目标和今天要做的事连在一起 |
...
如果你用 **XMind** 管理长期目标（本项目的 TAIE 脑图），在节点上标 **红旗** 表示「当前要推进的事」——不用每天手动抄进任务清单。

**怎么用：**

1. 把脑图文件放到安装目录的 `data\taie\TAIE.xmind`（见 [安装指南](docs/INSTALL.md)）
2. 照常对 OpenClaw 说：**「今天做什么」**

**会发生什么：**

- 自动读取脑图里**所有红旗任务**
- 同步进今日任务池（`Today / Active`），并写进 `TODAY.md` 的「TAIE 红旗」区块
- 你在脑图里更新红旗后，下次问「今天做什么」会**按最新脑图刷新**，避免清单和导图各说各话

**你可以这样说：**

| 你说 | 效果 |
|------|------|
| 今天我应该先做什么？ | 今日计划里会带上脑图红旗，和口头记下的任务一起看 |
| 脑图里有哪些红旗？ | 列出当前 XMind 里标红的待办 |
| 记一下：下午交材料 | 和脑图无关的临时任务照常记录，与红旗任务并存 |

> **没有 XMind 也能用**：不放脑图文件时，记任务、今日计划、提醒等功能完全正常；联动是加分项，不是必选项。
...
若已配置 `TAIE.xmind`，计划里还会带上**脑图红旗**，长期目标和当天行动在同一份「今天」视图里。
...
|-------------|-------------------|
| 记一下：下周买打印机 | 记录任务 |
| 今天做什么 / 我上班了 | 日切 + 今日计划（含 XMind 红旗同步） |
| 脑图里有哪些红旗 / TAIE 红旗 | 读取 XMind 当前红旗列表 |
| 还有什么没做 | 列出未完成任务 |
| 做完了，周报那个 | 标记完成 |
| 提醒我三点开会 | 记录 + 定时提醒流程 |
| 检查一下任务系统 | 健康检查 |
...
**XMind 脑图怎么接上？**  
把 `.xmind` 文件放到 `你的工具路径\data\taie\TAIE.xmind`，问「今天做什么」或「脑图红旗」即可。脑图仍在你本机用 XMind 编辑，Skill 只读取、不修改 `.xmind`。

**没有 TAIE 脑图能用吗？**  
可以。脑图是可选亮点；没有时只是「红旗 ↔ 今日任务」联动不可用，记任务、今日计划、提醒等照常。
